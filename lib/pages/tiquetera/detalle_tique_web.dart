...
// Imports condicionales para evitar errores en Android/iOS
import 'package:universal_html/html.dart' as html; 
// O si prefieres no usar universal_html, usa:
// import 'package:vientri/services/web_audio_stub.dart' if (dart.library.html) 'dart:html' as html;
...
class MyBytesSource extends StreamAudioSource {
  ...
  MyBytesSource(this._bytes, {this.contentType = 'audio/webm'}); 
...
class DetalleTiqueWeb extends StatefulWidget {
  Entidad entidad;
  int idTique;
  ...
  @override
  State<DetalleTiqueWeb> createState() => _DetalleTiqueWebState();
}

class _DetalleTiqueWebState extends State<DetalleTiqueWeb> {
  late Controller con;
  ...
  void initState() {
    ...
    _cargarInicial();
    _desbloquearAudioWeb();
  }
  
  Future<void> _activarAudioWeb() async {
    if (_audioActivado) return;
    try {
      // Cargamos un mini buffer de silencio para que el navegador de permiso
      await _audioPlayer.setAudioSource(MyBytesSource(Uint8List.fromList([0,0,0,0]))); 
      await _audioPlayer.play();
      await _audioPlayer.stop();
      _audioActivado = true;
      print("AudioContext activado por el usuario");
    } catch (e) {
      print("No se pudo activar audio aún: $e");
    }
  }

  void _desbloquearAudioWeb() {
    if (kIsWeb) {
      html.window.onClick.first.then((_) {
        _audioPlayer.play(); 
        _audioPlayer.stop();
        print("AudioContext desbloqueado");
      });
    }
  }
  ...
  Future<void> _cargarInicial() async {
    ...
    if (detalle.usuarioSolicitante?.telefono != null) {
  ...
  Uint8List base64ToBytes(String base64String) {
    // 1. Quitar encabezados data:audio...
    String cleaned = base64String.contains(',') ? base64String.split(',').last : base64String;
    // 2. IMPORTANTE para Release: Quitar saltos de línea y espacios que mete el servidor
  ...
  Future<void> _playBase64(String base64, String mensajeId) async {
    try {
      ...
      // "Despertamos" el audio (User Gesture)
      await _activarAudioWeb();

      await _audioPlayer.stop();
      
      // Cambiamos a mpeg que es más compatible en Web compilada que webm
      await _audioPlayer.setAudioSource(
        MyBytesSource(bytes, contentType: 'audio/mpeg'), 
        preload: true,
      );

      // RECIÉN AQUÍ activamos la animación, cuando el player está listo
      _audioIdActivo.value = mensajeId;
      _audioPlaying.value = true;
      _audioIdEnPlayer = mensajeId;
      
      await _audioPlayer.play();
    } catch (e) {
      // Si hay error, nos aseguramos de que la animación se apague
      _audioPlaying.value = false;
      _audioIdActivo.value = null;
  ...
  Future<void> _startRecording() async {
    try {
      ...
      // Determinamos el codec según la plataforma
      // WebM Opus es el formato estándar y más soportado en navegadores
      Codec codecWeb = Codec.opusWebM; 
      Codec codecMovil = Codec.aacADTS;

      await _recorder.startRecorder(
        toFile: kIsWeb ? 'audio_record.webm' : 'audio_record.aac',
        codec: kIsWeb ? codecWeb : codecMovil,
      );
      ...
      // Si falla Opus, podrías intentar con un fallback a MP3 o AAC si el navegador lo permite
    }
  }
  ...
  Future<String> _stopAndGetBase64() async {
    ...
    // El path en web es una URL de un Blob (ej: blob:http://localhost:123/...)
    ...
    if (kIsWeb) {
      // WEB: Descargamos los bytes desde la URL del Blob generada por el recorder
      ...
      // MÓVIL: Aquí podrías usar File, pero para evitar el error de compilación
      // lo ideal es usar una librería como 'cross_file' (XFile)
      // o importar dart:io condicionalmente.
      return ""; // Implementar con XFile si necesitas soporte móvil en el mismo archivo
    }

    _transcribiendo.value = true;
    // Enviamos los bytes codificados a tu controlador
                    ...
                    children: [
                      Expanded(
                        child: SubtleButton(
                          ...
                          text: "Cerrar sesión",
                          onPressed: () {
                            GetStorage().write('pending_route', '/web/tiquet/${widget.idTique}');
  ...
  Widget _contenido(BuildContext context, TiqueDetalle tiqueDetalle) {
    ...
    return PopScope(
      canPop: !accessedViaWebLink,
      onPopInvoked: (didPop) {
        if (accessedViaWebLink && !didPop) {
          // Mostrar mensaje si intentan navegar hacia atrás
          con.mostrarSnackbar(
            titulo: "Acceso restringido",
            mensaje: "No puedes navegar hacia atrás desde este enlace",
            esError: false,
          );
        }
      },
      child: MasterPage(
        ...
        onBack: accessedViaWebLink ? null : () => Navigator.pop(context, true),
        showKey: false,
        showMore: true,
        ...
        child: Obx(
          () => Column(
            children: [
              ...
              else if (!_isRecording.value && _resumen.value.isNotEmpty)
              Container(
