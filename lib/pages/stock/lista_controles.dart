...
class ListaControles extends StatefulWidget {
  Entidad entidad;
  ListaControles({super.key, required this.entidad});

  @override
  State<ListaControles> createState() => _ListaControlesState();
}

class _ListaControlesState extends State<ListaControles> {
  final LoginController con = LoginController();
  late Controller cont;
  late Future<List<Control>> _futureControles;
  final cronometro = Get.put(CronometroController());
  Timer? _debounce;
  List<Opcion> _opcionesSalones = [];
  late Opcion ubicacionSeleccionada;
  ...
  void initState() {
    ...
    cronometro.setControl(Control(id: 0, diferencias: "", ubicacion: "", ubicacionId: 0, usrSolicita: "", idUsrSolicita: 0, empleado: "", empleadoId: 0, fecha: "", hora: "", estadoId: 0, duracion: "", horaIni: "", horaFin: "", items: 0, articulos: []));
    _futureControles = cont.listaControles();
    ...
    if(cont.contienePermiso(218)) {
      _opcionesSalones.add(Opcion(id: -1, nombre: "Todos"));
      ubicacionSeleccionada = Opcion(id: -1, nombre: "Todos");
      _opcionesSalones.sort((b, a) => b.id.compareTo(a.id));
    } else {
      ubicacionSeleccionada = widget.entidad.salones.map((item) {
        var parts = item.split(' - ');
        return Opcion(
          id: int.parse(parts[0]),
          nombre: parts[1],
  ...
  void abrirBusqueda() {
    ...
    SearchBarPopup.show(
      context,
      hintText: "Buscar control",
      controller: popupController,
      onChanged: (value) {
        if (_debounce?.isActive ?? false) _debounce!.cancel();
        _debounce = Timer(const Duration(seconds: 2), () {
          _futureControles = cont.listaControles(value);
        });
      },
    );
  }
    ...
    return MasterPage(
      ...
      ? SolidButton(
        ...
        onPressed: () {
          Navigator.of(context).push(
            PageRouteBuilder(
              pageBuilder: (context, animation, secondaryAnimation) => AsignarControl(entidad: widget.entidad),
              transitionsBuilder: (context, animation, secondaryAnimation, child) {
                const begin = Offset(1.0, 0.0);
                const end = Offset.zero;
          ...
          InkWell(
            onTap: () {
              ActionSheetOptions.show(
                ...
                onOptionSelected: (s) {
                  setState(() {
                    if (s.id != -1) {
                      ubicacionSeleccionada.nombre = cont.capitalizar(s.nombre);
                      ubicacionSeleccionada.id = s.id;
                    }
                    ubicacionSeleccionada.nombre = s.nombre;
                    ubicacionSeleccionada.id = s.id;
                    _futureControles = cont.listaControles();
                  });
                },
            ...
            child: Row(
              children: [
                ...
                Text(
                  cont.capitalizarNombre(ubicacionSeleccionada.nombre),
                  style: TextStyle(
          ...
          RefreshIndicator(
            ...
            child: FutureBuilder<List<Control>>(
              ...
              builder: (context, snapshot) {
                ...
                var vigentes = pedidos
                    .where((p) => p.estadoId != 10824 && p.empleadoId == widget.entidad.usuarioId && p.ubicacionId == ubicacionSeleccionada.id)
                    .toList()
                ...
                if (cont.contienePermiso(218)) {
                  if (ubicacionSeleccionada.id != -1) {
                    vigentes = pedidos
                      .where((p) => p.estadoId != 10824 && p.ubicacionId == ubicacionSeleccionada.id)
                      .toList()
                    ..sort((a, b) => b.id.compareTo(a.id));
                          ...
                          ...vigentes.map((v) => v.estadoId == 10823
                            ? CronometroListedElement(control: v, funcion: () {
                                if (cont.contienePermiso(217) || v.empleadoId == widget.entidad.usuarioId) {
                                  Navigator.of(context).push(
                                    PageRouteBuilder(
                                      pageBuilder: (context, animation, secondaryAnimation) => NuevoControl(entidad: widget.entidad, control: v),
                                      transitionsBuilder: (context, animation, secondaryAnimation, child) {
                                        const begin = Offset(1.0, 0.0);
                                        const end = Offset.zero;
                                        final tween = Tween(begin: begin, end: end).chain(CurveTween(curve: Curves.ease));
      ...
      children: [
        TableRow(
          children: [
            ...
            control.estadoId == 10824 && cont.contienePermiso(218)
                ? _cellIcon(
